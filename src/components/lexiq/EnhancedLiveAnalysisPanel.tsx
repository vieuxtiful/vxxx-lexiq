                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between gap-2 pb-2 border-b">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      {hoveredTerm.ui_metadata?.confidence_level && <Badge variant="outline" className="text-xs">
                  ...
                  {/* Grammar Issues */}
                  {hoveredTerm.grammar_issues && hoveredTerm.grammar_issues.length > 0 && <div>
                      <div className="text-xs text-muted-foreground mb-1">Grammar Issues:</div>
                      <div className="space-y-1">
                        {hoveredTerm.grammar_issues.map((issue, idx) => <div key={idx} className="text-xs bg-muted p-2 rounded">
                  ...
                  {hoveredTerm.suggestions && hoveredTerm.suggestions.length > 0 && hoveredTerm.suggestions.some(s => s !== hoveredTerm.text) && <div>
          ...
          top: `${clickPosition.y}px`
              ...
              <div className="bg-popover border-2 border-primary rounded-lg shadow-xl p-3 min-w-[200px]">
                <div className="space-y-2">
                  ...
                  {clickedTerm.suggestions && clickedTerm.suggestions.length > 0 && <div className="space-y-1">
                      <div className="text-xs text-muted-foreground">Replace with:</div>
                      {clickedTerm.suggestions.filter(suggestion => suggestion !== clickedTerm.text).slice(0, 3).map((suggestion, idx) => <Button key={idx} variant="ghost" size="sm" className="w-full justify-start text-xs h-7" onClick={() => {
                  const newContent = content.slice(0, clickedTerm.position.start) + suggestion + content.slice(clickedTerm.position.end);
                  onContentChange(newContent);
                  setClickedTerm(null);
                  setClickPosition(null);
                }}>
                  ...
                  {clickedTerm.classification === 'grammar' && clickedTerm.grammar_issues && <div className="space-y-1">
                      <div className="text-xs text-muted-foreground">Grammar fixes:</div>
                      {clickedTerm.grammar_issues.slice(0, 2).map((issue, idx) => <Button key={idx} variant="ghost" size="sm" className="w-full justify-start text-xs h-7 text-purple-600" onClick={() => {
                  ...
                  {clickedTerm.classification === 'review' && onValidateTerm && <div className="pt-2 border-t">
                      <Button variant="ghost" size="sm" className="w-full justify-start text-xs h-7 text-green-600 hover:text-green-700 hover:bg-green-50" onClick={() => {
                  ...
                  <div className="pt-2 border-t">
                    <Button variant="ghost" size="sm" className="w-full text-xs h-7" onClick={() => {
                  setClickedTerm(null);
                  setClickPosition(null);
                }}>
                      Dismiss
